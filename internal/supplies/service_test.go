...
func (m *mockTenantRepository) GetByTaxID(
	ctx context.Context,
	taxID string,
) (*models.Tenant, error) {
...
func TestCreateSupply_Success(t *testing.T) {
	...
	repo := &mockRepository{
		getByNameFunc: func(
			...
			if name != "Pan Brioche" {
				t.Errorf(
					"se esperaba nombre normalizado, se obtuvo: %s",
					name,
				)
			}
	...
	if err != nil {
		t.Fatalf(
			"se esperaba exito, se obtuvo error: %v",
			err,
		)
	}

	if res.Name != "Pan Brioche" ||
		res.CurrentWholesaleCost != 100.5 ||
		res.CurrentStock != 25.75 ||
		res.MeasurementUnit != "kg" ||
		!res.IsActive {

		t.Errorf(
			"los datos no se normalizaron correctamente: %+v",
			res,
		)
	}
}
...
func TestCreateSupply_InvalidData(t *testing.T) {
	...
	if !errors.Is(err, ErrInvalidSupplyData) {
		...
		t.Errorf(
			"se esperaba ErrInvalidSupplyData, se obtuvo: %v",
			err,
		)
	}
}
...
func TestCreateSupply_InvalidCost(t *testing.T) {
	...
	if !errors.Is(err, ErrInvalidSupplyCost) {
		...
		t.Errorf(
			"se esperaba ErrInvalidSupplyCost, se obtuvo: %v",
			err,
		)
	}
}
...
func TestCreateSupply_InvalidStock(t *testing.T) {
	...
	if !errors.Is(err, ErrInvalidSupplyStock) {
		...
		t.Errorf(
			"se esperaba ErrInvalidSupplyStock, se obtuvo: %v",
			err,
		)
	}
}
...
func TestCreateSupply_DuplicateName(t *testing.T) {
	...
	if !errors.Is(err, ErrDuplicateSupplyName) {
		...
		t.Errorf(
			"se esperaba ErrDuplicateSupplyName, se obtuvo: %v",
			err,
		)
	}
}
...
func TestListSupplies_TenantNotFound(t *testing.T) {
	...
	if !errors.Is(err, ErrTenantNotFoundForSupply) {
		...
		t.Errorf(
			"se esperaba ErrTenantNotFoundForSupply, se obtuvo: %v",
			err,
		)
	}
}
...
func TestUpdateSupply_Success(t *testing.T) {
	...
	if res.Name != "Queso" ||
		res.CurrentWholesaleCost != 150 ||
		res.CurrentStock != 8.5 ||
		res.MeasurementUnit != "un" ||
		res.IsActive {

		t.Errorf(
			"los datos no se actualizaron correctamente: %+v",
			res,
		)
	}
}
...
func TestDeleteSupply_NotFound(t *testing.T) {
	...
	if !errors.Is(err, ErrSupplyNotFound) {
		...
		t.Errorf(
			"se esperaba ErrSupplyNotFound, se obtuvo: %v",
			err,
		)
	}
}
